...
const Cubes: React.FC<CubesProps> = ({
    ...
    // Cache the DOM nodes to avoid extremely expensive querySelectorAll calls 60 times a second
    const cubesDataRef = useRef<Array<{ el: HTMLDivElement; r: number; c: number }>>([]);

    // Initialize cubes cache
    useEffect(() => {
        if (!sceneRef.current) return;
        const cubes = Array.from(sceneRef.current.querySelectorAll<HTMLDivElement>('.cube'));
        cubesDataRef.current = cubes.map(cube => ({
            el: cube,
            r: parseInt(cube.dataset.row || "0", 10),
            c: parseInt(cube.dataset.col || "0", 10)
        }));
    }, [gridSize]);
    ...
    const tiltAt = useCallback(
        (rowCenter: number, colCenter: number) => {
            ...
            cubesDataRef.current.forEach(({ el: cube, r, c }) => {
                ...
                if (dist <= radius) {
                    ...
                    gsap.to(cube, {
                        duration: enterDur,
                        ease: easing,
                        overwrite: "auto", // Much faster than true
                        rotateX: -angle,
                        rotateY: angle
                    });
                } else {
    ...
    const onPointerMove = useCallback(
        (e: MouseEvent) => {
            ...
            if (now - lastPointerMove.current < 45) return; // Throttle to ~22fps for cube updates
    ...
    const resetAll = useCallback(() => {
        if (!cubesDataRef.current.length) return;
        cubesDataRef.current.forEach(({ el: cube }) =>
            gsap.to(cube, {
                duration: leaveDur,
                rotateX: 0,
                rotateY: 0,
    ...
    useEffect(() => {
        ...
        const loop = (time: number) => {
            if (!userActiveRef.current) {
                ...
                // Throttle the tilt updates heavily so we don't dispatch GSAP commands 60 times a second
                if (time - lastSimTime > 100) {
                    tiltAt(pos.y, pos.x);
                    lastSimTime = time;
                }
    ...
    useEffect(() => {
        ...
        // Ensure TS understands the exact types
